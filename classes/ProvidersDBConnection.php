...
class ProvidersDBConnection {
    ...
    public function getTractsByZip($zip){
        $table_name = $this->wpdb->prefix . "zip_tract";
        $tracts_query = "SELECT tract FROM $table_name WHERE zip = '$zip'";
    ...
    public function getCoverage($tracts, $total, $provider) {

        $possible_names_arr = get_field('possible_provider_names', $provider);
        $possible_name= [];
        ...
        //error_log(print_r($possible_names, TRUE));

        $sql = "SELECT hso_provider, census_block_fips_code_11 FROM $this->internet_providers_table_name WHERE census_block_fips_code_11 IN ($tracts) and hso_provider REGEXP '$possible_names'";
        $fcc_provider_return = $this->wpdb->get_results($sql);
        
        $final = [];
        foreach ($fcc_provider_return as $current) {
            if ( ! in_array($current, $final)) {
                $final[] = $current;
            }
        }
        //error_log(count($final));
        $count = count($final);
