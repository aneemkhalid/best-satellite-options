...
function splitMaxDownUpload($key_array) {
    ...
    //Filter Result variable missing might need to fix at some point
    ...
    // } else {
        $filtered = array_filter(array_column($SpeedArray, 1));
        $MinSpeed = $MaxSpeed = '';
        if($filtered) {
            $MinSpeed = $filtered[0];
            $MaxSpeed = $filtered[0];
        ...
        $keyMax = array_search($MaxSpeed, array_column($SpeedArray, 1));
    //}
    if ( $MinSpeed == $MaxSpeed && $MinSpeed !== ''){ // BOTH ARE EQUAL
