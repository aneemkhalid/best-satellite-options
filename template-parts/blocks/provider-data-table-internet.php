...
foreach( $Providers as $Provider ){
    ...
    if($ConntArr && in_array('satellite', $ConntArr)) {
        $is_satellite = true;
    }
    ...
    $providersListProdClick = '';
        ...
        if ( $TableStyle == "minimal-table" ){
            ...
            $beginning = ($is_satellite) ? '<a href="' . $Detail_Table[$Ind]['provider']['URL'] . '" onclick="'.$providersListProdClick.'">' : '<div>';
            $end = ($is_satellite) ? '</a>' : '</div>';
            $format = '%s ' . $Detail_Table[$Ind]['provider']['name'] . ' %s';
            $Detail_Table[$Ind]['provider']['cell_data'] = sprintf($format, $beginning, $end);
        }else{
